...
module.exports = function(client) {
  ...
  return {
    ...
    getAll: function(options, allRepos) {
      ...
      return client.getCollection('repos', options).then(function(repos) {
        if (!repos.isLastPage) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          projects
            ...
            .then(function(projectsResponse) {
              ...
              async.forEachOf(
                projectsResponse.values,
                function(project, index, callback) {
                  // Get project repos.
                  API.get(project.key)
                    .then(function(repoResponse) {
                      // Merge to result.
                      reposCombined = utils(reposCombined)
                        .concat(repoResponse.values)
                        .value();
                      callback();
                    })
                    .catch(function(err) {
    ...
    post: function(projectKey, data) {
      return client.post('projects/' + projectKey + '/repos', data);
    },

    put: function(projectKey, repositorySlug, data) {
      return client.put(
